...
struct GenfitTrackFitter_v2 final : 
...
{
    GenfitTrackFitter_v2(const std::string& name, ISvcLocator* svcLoc) : 
                ...
                extension::TrackCollection> operator()( const extension::TrackCollection& tracks_input) const override                                                 
    {
        ...
        for (const auto& track : tracks_input)
        {
            ...
            if (track.getTrackerHits().size() < 3) continue; // skip tracks with less than 3 hits
            num_tracks  +=1;
            ...
            for (int pdgCode : m_particleHypotesis)
            {
                ...
                int q = getHypotesisCharge(pdgCode);
                // Initialize the edm4hepTrack_
                auto edm4hepTrack_ = extension::MutableTrack();
                ...
                int middle_hit_index = edm4hepTrack_.getTrackerHits().size() / 2;
                ...
                auto middleHit_position = edm4hepTrack_.getTrackerHits(middle_hit_index).getPosition();
                auto lastHit_position = edm4hepTrack_.getTrackerHits(-1).getPosition();

                TVector3 firstHit_referencePoint = TVector3(dd4hep::mm * firstHit_position.x, dd4hep::mm * firstHit_position.y, dd4hep::mm * firstHit_position.z);
                TVector3 middleHit_referencePoint = TVector3(dd4hep::mm * middleHit_position.x, dd4hep::mm * middleHit_position.y, dd4hep::mm * middleHit_position.z);
                TVector3 lastHit_referencePoint = TVector3(dd4hep::mm * lastHit_position.x, dd4hep::mm * lastHit_position.y, dd4hep::mm * lastHit_position.z);

                // // Alias
                // TVector3 p1 = firstHit_referencePoint;
                // TVector3 p2 = middleHit_referencePoint;
                // TVector3 p3 = lastHit_referencePoint;
                ...
                // if (std::abs(d) < 1e-10) {
                //     std::cerr << "Error: aligned hits." << std::endl;
                //     num_tracks -=1;
                //     continue;
                // }
                ...
                // double xc = (x1s*(y2 - y3) + x2s*(y3 - y1) + x3s*(y1 - y2)) / d;
                // double yc = (x1s*(x3 - x2) + x2s*(x1 - x3) + x3s*(x2 - x1)) / d;

                // TVector3 center(xc, yc, 0.0);
                // double radius = std::sqrt((x1 - xc)*(x1 - xc) + (y1 - yc)*(y1 - yc));

                // TVector3 r = p1 - center;
                // TVector3 pT_dir(-r.Y(), r.X(), 0); 
                // if (q > 0)
                //     pT_dir = TVector3(-r.Y(), r.X(), 0);
                // else
                //     pT_dir = TVector3(r.Y(), -r.X(), 0);
                // pT_dir = pT_dir.Unit();     

                // TVector3 r3 = p3 - center;
                // double phi1 = std::atan2(r.Y(), r.X());
                // double phi3 = std::atan2(r3.Y(), r3.X());
                // double dphi = phi3 - phi1;

                // double pi = std::numbers::pi;
                // if (dphi > pi) dphi -= 2*pi;
                // if (dphi < -pi) dphi += 2*pi;
                // double arcLength = std::abs(radius * dphi);
                // double dz = p3.Z() - p1.Z();
                // double tanTheta = dz / arcLength;

                // TVector3 init_mom(pT_dir.X(), pT_dir.Y(), tanTheta);
                // init_mom = init_mom.Unit();

                // // Output
                // std::cout << "Initial position: " << p1.X() << ", " << p1.Y() << ", " << p1.Z() << std::endl;
                // std::cout << "Initial momentum direction: "
                //         << init_mom.X() << ", "
                //         << init_mom.Y() << ", "
                //         << init_mom.Z() << std::endl;
                ...
                // GenfitInterface::GenfitTrack track_interface = GenfitInterface::GenfitTrack(track, dch_info, dc_decoder,pdgCode,init_pos,init_mom);
...
DECLARE_COMPONENT(GenfitTrackFitter_v2)
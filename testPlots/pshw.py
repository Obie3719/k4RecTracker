...
"""Overlay: number of PSHW (SOT+PSHW) hits per fitted track, per theta."""
...
# In this setup, use SiWrapper barrel+endcap as SOT+PSHW-like layers.
PSHW_DIGI_COLLECTIONS = ["SiWrBDigis", "SiWrDDigis"]
KEEP_ONLY_TRACKS_WITH_PSHW = False

X_MIN = 0.0
X_MAX = 9.0
N_BINS = 90

OUT_PNG = "pshw_hits_per_track.png"
...
def collect_hits_per_track(m, filename, active_digi_names):
    ...
    for event in events:
        ...
        pshw_oids = build_oid_set(m, event, PSHW_DIGI_COLLECTIONS)

        for trk in tracks:
            n_pshw = 0
            for hit in m.iter_tracker_hits(trk):
                if m.oid(hit) in pshw_oids:
                    n_pshw += 1

            if KEEP_ONLY_TRACKS_WITH_PSHW and n_pshw <= 0:
                continue

            counts.append(n_pshw)
            if n_pshw > 0:
                summary["tracks_with_pshw"] += 1
...
def main():
    ...
    print("PSHW digi collections:", PSHW_DIGI_COLLECTIONS)
    print("Keep only tracks with PSHW hits:", KEEP_ONLY_TRACKS_WITH_PSHW)
    ...
    for subdir in subdirs:
        ...
        for file_path in fit_files:
            ...
            if vals.size == 0:
                print(f"  -> no PSHW-hit track values in {fname}")
                continue
            ...
            print(
                ...
                f"tracks_with_pshw={summary['tracks_with_pshw']}, "
                f"in_plot_range={in_range}"
            )

    if not vals_by_theta:
        raise RuntimeError("No PSHW-hit values collected. Check inputs/collections/cuts.")
    ...
    plt.title("Number of pshw hits per tracks", fontsize=14)
    plt.xlabel("Number of pshw hits per tracks", fontsize=12)

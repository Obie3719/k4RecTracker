...
def parse_args():
    parser = argparse.ArgumentParser(description="Muon reconstruction efficiency vs eta.")
    parser.add_argument("-i", "--input", default=common.INPUT_FILE, help="Input PODIO root file.")
    parser.add_argument(
        "-o", "--output", default="efficiency_vs_eta.png", help="Output plot filename."
    )
    parser.add_argument(
...
def main():
    ...
    totals = np.zeros(len(ETA_BINS_EFF) - 1)
    selected = np.zeros(len(ETA_BINS_EFF) - 1)
    ...
    for _, event in common.iter_events(events, args.max_events):
        ...
        for t in truth_muons:
            ...
            if vec["pt"] < args.pt_min or abs(vec["eta"]) > args.eta_max:
                continue
            ib = np.digitize(vec["eta"], ETA_BINS_EFF) - 1
            if 0 <= ib < len(totals):
                totals[ib] += 1
